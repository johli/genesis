...
def load_saved_predictor(model_path, library_contexts=None) :
	...
	def _load_predictor_func(sequence_input, sequence_class) :
		...
		if library_contexts is None :
			...
			#lib_input = Lambda(lambda x: K.tile(K.gather(K.constant(sequence_class_libs), K.cast(x[0][:, 0], dtype='int32')), (K.shape(x[1])[0] / K.shape(x[0])[0], 1)))([sequence_class, sequence_input])

		#distal_pas_input = Input(tensor=K.ones(distal_pas_shape))
		distal_pas_input = Lambda(lambda x: K.tile(K.constant(np.ones((1, 1))), (K.shape(x)[0], 1)))(sequence_input)
