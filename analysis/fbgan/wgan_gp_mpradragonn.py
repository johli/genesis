...
class WGAN_LangGP():
    def __init__(self, batch_size=64, lr=0.0001, num_epochs=150, seq_len = 145, data_dir='./data/mpradragonn_seqs_strong.txt', \
        run_name='mpradragonn_strong', hidden=512, d_steps = 10):
        self.hidden = hidden
    ...
    def load_model(self, directory = ''):
        ...
        if len(list_G) == 0:
            ...
            return 0 #return 1 #file is not there
    ...
    def disc_train_iteration(self, real_data):
        ...
        return d_err, d_fake_err.data, d_real_err.data, gradient_penalty.data
    ...
    def train_model(self, load_dir):
        counter = self.load_model(load_dir)
        n_batches = int(len(self.data)/self.batch_size)
        total_iterations = self.n_epochs * n_batches#4000
        ...
        init_epoch = int(counter / n_batches)
        init_idx = int(counter % n_batches)
        
        print("Starting from counter = " + str(counter))
        print(" - epoch = " + str(init_epoch))
        print(" - data idx = " + str(init_idx))

        #counter = 0
        for epoch in range(init_epoch, self.n_epochs):
            print("Current epoch = " + str(epoch))
            
            for idx in range(init_idx, n_batches):
                _data = np.array(
                ...
                d_err, d_fake_err, d_real_err, gradient_penalty = self.disc_train_iteration(real_data)
...
def main():
    ...
    parser.add_argument("--run_name", default= "mpradragonn_strong", help="Name for output files (checkpoint and sample dir)")
