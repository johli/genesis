...
def load_saved_predictor(model_path, library_contexts=None) :
	...
	def _initialize_predictor_weights(predictor_model) :
		#Load nmer weights
		nmer_weights = pd.read_csv(model_path + 'alt_5ss_6mers.csv', sep='\t')
	...
	def _load_predictor_func(sequence_input, sequence_class) :
		...
		mer6_count = Lambda(lambda x: K.sum(x[:, 5:40-5, :, :], axis=(1, 2)))(mer6_out)
		mer4_count = Lambda(lambda x: K.sum(x[:, 7:38-3, :, :], axis=(1, 2)))(mer4_out)
