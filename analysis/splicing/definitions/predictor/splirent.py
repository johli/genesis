...
def load_saved_predictor(model_path, library_contexts=None) :
	...
	def _load_predictor_func(sequence_input, sequence_class) :
		...
		shared_out = shared_model(Lambda(lambda x: x[:, :, :, 0])(sequence_input))
		...
		layer_usage_hek = Dense(1, activation='sigmoid', kernel_initializer='zeros', name='splirent_hek_dense')
		layer_usage_hela = Dense(1, activation='sigmoid', kernel_initializer='zeros', name='splirent_hela_dense')
		layer_usage_mcf7 = Dense(1, activation='sigmoid', kernel_initializer='zeros', name='splirent_mcf7_dense')
		layer_usage_cho = Dense(1, activation='sigmoid', kernel_initializer='zeros', name='splirent_cho_dense')
